...
public class Pager {
	...
	public Pager(long totalSize) {
		...
		this.totalPage = (int) Math.ceil(totalSize / this.pageSize);
	...
	public Pager pagerFlip() {
		...
		if (pager.getPageNum() > pager.totalPage && pager.totalPage != 0) {
			pager.setPageNum(this.totalPage);
		} else if ((pager.getPageNum() <= 0 && pager.getTotalPage() > 0) || pager.totalPage == 0) {
			pager.setPageNum(1);
		}
		...
		this.log.debug("下一个分页信息：totalSize[" + pager.getTotalSize() + "] totalPage[" + pager.getTotalPage()
				+ "] pageSize[" + pager.getPageSize() + "] pageNum[" + pager.getPageNum() + "]");
